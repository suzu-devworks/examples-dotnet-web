...
openssl ecparam -genkey -name secp384r1 -noout -out ${DEST_DIR}/example.ca.key
openssl req -new -x509 -config ${CONF_FILE} -batch \
    -subj "/C=JP/O=examples/CN=Example Root CA" \
    -key ${DEST_DIR}/example.ca.key \
    -out ${DEST_DIR}/example.ca.crt -days ${DAYS} -extensions v3_ca
...
openssl x509 -req -in ${DEST_DIR}/example.signer.csr \
    -CA ${DEST_DIR}/example.ca.crt -CAkey ${DEST_DIR}/example.ca.key -CAcreateserial \
...
openssl pkcs12 -export -in ${DEST_DIR}/example.signer.crt -inkey ${DEST_DIR}/example.signer.key \
    -out ${DEST_DIR}/example.signer.p12 -passout file:${PASSWORD_FILE}
...
openssl pkcs12 -export -in ${DEST_DIR}/example.user-rsa.crt -inkey ${DEST_DIR}/example.user-rsa.key \
    -out ${DEST_DIR}/example.user-rsa.p12 -passout file:${PASSWORD_FILE}
...
openssl pkcs12 -export -in ${DEST_DIR}/example.user-ecdsa.crt -inkey ${DEST_DIR}/example.user-ecdsa.key \
    -out ${DEST_DIR}/example.user-ecdsa.p12 -passout file:${PASSWORD_FILE}
